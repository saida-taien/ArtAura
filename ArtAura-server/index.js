...
const corsConfig = {
  origin: ["http://localhost:5173" ,"https://art-aura.web.app" ],
  credentials: true,
};
...
async function run() {
  try {
    ...
    app.get("/categorySub/:subCategory", async (req, res) => {
...
app.get('/', (req, res) => {
  res.send("ArtAura server is running");
})

app.listen(port, () => {
  console.log(`ArtAura server is running on port : ${port}`);
})